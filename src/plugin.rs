...
pub struct ImageExporterSource(pub Handle<Image>);

impl From<Handle<Image>> for ImageExporterSource {
...
pub struct ImageExporterSettings {
...
pub struct GpuImageExporterSource {
    pub buffer: Buffer,
...
impl RenderAsset for ImageExporterSource {
    type ExtractedAsset = Self;
    type PreparedAsset = GpuImageExporterSource;
...
#[derive(Component, Clone)]
pub struct ImageExporterStartFrame(u64);

impl Default for ImageExporterSettings {
    fn default() -> Self {
        Self {
            output_dir: "out".into(),
            extension: "png".into(),
...
impl ExtractComponent for ImageExporterSettings {
    type Query = (
        &'static Self,
        &'static Handle<ImageExporterSource>,
        &'static ImageExporterStartFrame,
    );
    type Filter = ();
    type Out = (Self, Handle<ImageExporterSource>, ImageExporterStartFrame);
...
fn setup_exporters(
    ...
    exporters: Query<
        Entity,
        (
            With<ImageExporterSettings>,
            Without<ImageExporterStartFrame>,
        ),
    >,
    ...
    *frame_id = frame_id.wrapping_add(1);
    for entity in &exporters {
        commands
            .entity(entity)
            .insert(ImageExporterStartFrame(*frame_id));
    }
}

#[derive(Bundle, Default)]
pub struct ImageExporterBundle {
    pub source: Handle<ImageExporterSource>,
    pub settings: ImageExporterSettings,
}
...
fn save_buffer_to_disk(
    exporter_bundles: Query<(
        &Handle<ImageExporterSource>,
        &ImageExporterSettings,
        &ImageExporterStartFrame,
    )>,
    sources: Res<RenderAssets<ImageExporterSource>>,
    render_device: Res<RenderDevice>,
    export_threads: Res<ExportThreads>,
    mut frame_id: Local<u64>,
) {
    *frame_id = frame_id.wrapping_add(1);
    for (source_handle, settings, start_frame) in &exporter_bundles {
...
impl Plugin for ImageExportPlugin {
    fn build(&self, app: &mut App) {
        ...
        .add_asset::<ImageExporterSource>()
        .add_plugin(RenderAssetPlugin::<ImageExporterSource>::default())
        .add_plugin(ExtractComponentPlugin::<ImageExporterSettings>::default())
        .add_systems((
